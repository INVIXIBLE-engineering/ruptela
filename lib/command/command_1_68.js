            ...
            for (let f in recordFields.body) {
                ...
                if (n > 0) {
                    ...
                    for (let i = 0; i < n; ++i) {
                        ...
                        //set value for IO ID
                        if (record.ioIdString8.includes(ioId)) {
                            rec.io[ioId] = bufIt.readStringNext(8);
                        }
                        else if (record.ioIdString4.includes(ioId)) {
                            rec.io[ioId] = bufIt.readStringNext(4);
                        }
                        else if (record.ioIdString1.includes(ioId)) {
                            rec.io[ioId] = bufIt.readStringNext(1);
                        }
                        else if (record.ioIdSignedIntegers.includes(ioId)) {
                            rec.io[ioId] = bufIt.readNext(ioValueLength, false);
                        }
                        else {
                            rec.io[ioId] = bufIt.readNext(ioValueLength);
                        }
                    }
                }
